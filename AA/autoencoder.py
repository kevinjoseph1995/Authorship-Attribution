...
from keras.layers import Input, Dense, Conv1D, Conv2D, MaxPooling1D, Flatten, Dropout, UpSampling1D
...
x = Conv1D(128, 5, activation='relu', padding='same')(x)
# x = Dropout(0.3)(x)
encoded = MaxPooling1D(35)(x)  # global max pooling
print encoded.shape

x = Conv1D(128, 5, activation='relu', padding='same')(encoded)
x = UpSampling1D(2)(x)
x = Conv1D(128, 5, activation='relu', padding='same')(encoded)
x = UpSampling1D(2)(x)
decoded = Conv1D(128, 5, activation='relu', padding='same')(x)

from keras.models import Model
autoencoder = Model(sequence_input, decoded)
autoencoder.compile(optimizer='adadelta', loss='mse')
autoencoder.fit(x_train, x_train.reshape(1,x_train.shape[0],x_train.shape[1]), epochs=20, batch_size=128)
